...
class graph():
    def __init__(self, boardSize, width):
        ...
        self.dataLength = 20
        self.width = width
        self.increment = width/(self.dataLength)
        for i in range(0, self.dataLength):
            self.pointList.append([i*self.increment, self.boardSize+(150-(150/100)*0)])

    def addData(self, data):
        for i in self.pointList:
            i[0] -= self.increment

        self.pointList.append([self.width, self.boardSize+(150-(150/100)*data)])
        if len(self.pointList) > self.dataLength+1:
            self.pointList.pop(0)

    def drawSelf(self, screen):
        for i in range(0, 5):
            pygame.draw.line(screen, (50,50,50), (0, self.boardSize+i*(150/5)), (self.width, self.boardSize+i*(150/5)))

        for i in range(1, self.dataLength):
            pygame.draw.line(screen, (50,50,50), (self.increment*i, self.boardSize), (self.increment*i, self.boardSize + 150))

        try: pygame.draw.lines(screen, (255,0,0), False, self.pointList, 2)
        except: pass
...
def drawUI(boardSize, width, height, screen, nodes, buttons, activeNode):
    ...
    pygame.draw.rect(screen, (180, 180, 180), (0, boardSize, width, boardSize))
